...
static int tiles_is_winning_more_than_one_tile(tile_t tile_array[],
		int size_of_array, int had_pair) {
...
int tiles_plus_one_is_winning(tile_t holding[], int n, tile_t discard) {
	...
	return tiles_is_winning(tiles,
			tiles_get_count(tiles, MAX_HOLDING_COUNT + 1), 0);
}
...
const char * tiles_to_string(tile_t tiles[], int n, char * buf, int buffer_size) {
	if (buffer_size - 1< n)
		n = buffer_size - 1;
	buf[n] = 0;
	for (; n > 0; n--) buf[n-1] = (char)tiles[n-1];
	return buf;
}
