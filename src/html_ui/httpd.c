...
void cat(int client, FILE *resource)
{
 ...
 int n;
 while ((n = fread(data, 1, sizeof(data), resource)) > 0) send(client, data, n, 0);
}
...
int execute_game(int client, const char *path,
                 const char *method, const char *query_string)
{
 ...
 if (game_callback_ptr(path, query_string, big_buffer, big_buffer_size)) {
	 ...
	 send(client, buf, strlen(buf), 0);
	 strcpy(buf, "\r\n");
	 send(client, buf, strlen(buf), 0);	 send(client, big_buffer, strlen(big_buffer), 0);
	 return 1;
 }
 return 0;
}
...
int is_png_file(const char * filename) {
	...
	if (name_len > 4) {
		return (filename[name_len - 3] == 'p') && (filename[name_len - 2]
				== 'n') && (filename[name_len - 1] == 'g');
	}
	return 0;
}
/**********************************************************************/
